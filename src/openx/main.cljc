(ns openx.main
  "Kotodama WASM entrypoint for the Openx clean-room actor (L4) — Clojure port.
  ...
  facts (`openx.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"openx.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "openx-compat" :tier tier :entities entities} 200])
